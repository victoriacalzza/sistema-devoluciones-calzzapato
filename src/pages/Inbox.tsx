...
export default function Inbox() {
  ...
  }, [q, status, sucursal, marca])

  return (
    <div className="mx-auto max-w-[1400px] px-4 py-6 lg:px-8">
      <PageHeader
        title="Devoluciones"
        subtitle={`${rows.length} expedientes · Bandeja general`}
        actions={
          <>
                  ...
                  return (
                    ...
                    >
                      <td className="px-4 py-3">
                        ...
                        {r.outOfSla && <span className="ml-2 rounded bg-brand-50 px-1.5 py-0.5 text-[10px] font-semibold text-brand-600">SLA</span>}
                      </td>
                      <td className="px-4 py-3 text-slate-600">{RETURN_TYPES[r.tipo].short}</td>
                      <td className="px-4 py-3 text-slate-600">{r.sucursal}</td>
                      <td className="px-4 py-3 text-slate-600">{r.cliente}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="font-mono text-[11px] text-slate-400">{r.product.sku}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap text-slate-500">{r.fechaCreacion}</td>
                      <td className="px-4 py-3"><StatusBadge status={r.status} /></td>
                      <td className="px-4 py-3"><PriorityBadge priority={r.priority} /></td>
                      <td className="px-4 py-3"><Avatar person={resp} size="sm" /></td>
                    </tr>
                  )
                })}
              </tbody>
