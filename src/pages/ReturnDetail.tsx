...
// Destino comercial del producto al rechazar (el producto no regresa a CEDIS).
const DESTINOS_COMERCIALES = ['Remate', 'Promoción', 'Venta física regular']
...
export default function ReturnDetail() {
  ...
  // Rechazo: destino comercial del producto + retiro del canal Ecommerce.
  const [destinoComercial, setDestinoComercial] = useState<string>(data?.destinoComercial ?? '')
  const [quitarEcommerce, setQuitarEcommerce] = useState(false)
  const [bloqueadoEcommerce, setBloqueadoEcommerce] = useState(!!data?.bloqueadoEcommerce)
  const [ventaFisica, setVentaFisica] = useState(!!data?.ventaFisica)
  const [confirmEcommerceOpen, setConfirmEcommerceOpen] = useState(false)
  ...
  function confirmReject() {
    if (!rejectReason || !destinoComercial) return
    // El producto no regresa a CEDIS: permanece disponible para venta física.
    setVentaFisica(true)
    setBloqueadoEcommerce(quitarEcommerce)
    transition('rechazado', `rechazó la devolución — ${rejectReason} · destino comercial: ${destinoComercial}`, 'danger', `Devolución rechazada · destino: ${destinoComercial}`)
    if (quitarEcommerce) logEvent('quitó el producto del canal Ecommerce (permanece disponible para venta física)', 'status')
    setRejectOpen(false)
  }

  // Acción independiente (solo Compras): retirar del canal Ecommerce un producto
  // ya rechazado que permanece en tienda para venta física.
  function quitarDeEcommerce() {
    setBloqueadoEcommerce(true)
    setVentaFisica(true)
    setConfirmEcommerceOpen(false)
    logEvent('quitó el producto del canal Ecommerce (permanece disponible para venta física)', 'status')
    setBanner({ kind: 'info', text: 'Producto retirado del canal Ecommerce · permanece disponible para venta física.' })
  }
  ...
  return (
    <div className="pb-10">
      ...
      <div className="mx-auto max-w-[1400px] px-4 py-6 lg:px-8">
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          ...
          <div className="space-y-4">
            ...
            {/* Disposición del producto rechazado */}
            {(status === 'rechazado' || bloqueadoEcommerce) && (
              <Card>
                ...
                <div className="space-y-2">
                  {status === 'rechazado' && (
                    <div className="flex items-center gap-2 rounded-lg bg-rose-50 px-3 py-2 text-sm font-medium text-rose-700">
                      <X className="h-4 w-4" /> Devolución rechazada
                    </div>
                  )}
                  {(destinoComercial || data.destinoComercial) && (
                    <div className="flex items-center gap-2 rounded-lg bg-slate-100 px-3 py-2 text-sm font-medium text-slate-700">
                      <Store className="h-4 w-4" /> Destino: {destinoComercial || data.destinoComercial}
                    </div>
                  )}
                  {bloqueadoEcommerce && (
                    <div className="flex items-center gap-2 rounded-lg bg-rose-50 px-3 py-2 text-sm font-medium text-rose-700">
                      <Ban className="h-4 w-4" /> No disponible en Ecommerce
                    </div>
                  )}
                  {ventaFisica && (
                    <div className="flex items-center gap-2 rounded-lg bg-emerald-50 px-3 py-2 text-sm font-medium text-emerald-700">
                  ...
                  {data.resolucionRechazo && !destinoComercial && (
                    <p className="text-xs text-slate-500">{data.resolucionRechazo}</p>
                  )}
                  {/* Acción independiente (solo Compras / Administrador) */}
                  {role === 'compras' && status === 'rechazado' && !bloqueadoEcommerce && (
                    <Button className="mt-1" variant="danger" icon={<Ban className="h-4 w-4" />} onClick={() => setConfirmEcommerceOpen(true)}>
                      Quitar de existencia Ecommerce
                    </Button>
                  )}
                </div>
              </Card>
      ...
      {/* Motivo de rechazo + destino comercial */}
      {rejectOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-6" onClick={() => setRejectOpen(false)}>
          <div className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-2xl bg-white p-6 shadow-pop" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-2">
              <X className="h-5 w-5 text-rose-600" />
              <h3 className="text-base font-semibold text-slate-900">Rechazar devolución</h3>
            </div>

            {/* 1. Motivo de rechazo */}
            <p className="mt-3 text-sm font-medium text-slate-700">Motivo del rechazo<span className="ml-0.5 text-brand-600">*</span></p>
            <div className="mt-2 space-y-2">
              {REJECT_REASONS.map((r) => (
                <button
                  ...
                  className={cn(
                    'flex w-full items-center justify-between rounded-xl border px-4 py-2.5 text-left text-sm hover:border-rose-300 hover:bg-rose-50/40',
            ...
            {/* 2. Destino comercial del producto */}
            <p className="mt-5 text-sm font-medium text-slate-700">Destino comercial del producto<span className="ml-0.5 text-brand-600">*</span></p>
            <p className="text-xs text-slate-500">El producto no regresa a CEDIS: define su destino comercial.</p>
            <div className="mt-2 grid grid-cols-1 gap-2">
              {DESTINOS_COMERCIALES.map((d) => (
                <button
                  key={d}
                  onClick={() => setDestinoComercial(d)}
                  className={cn(
                    'flex w-full items-center justify-between rounded-xl border px-4 py-2.5 text-left text-sm hover:border-brand-300 hover:bg-brand-50/40',
                    destinoComercial === d ? 'border-brand-300 bg-brand-50 font-medium text-brand-700' : 'border-slate-200 text-slate-700',
                  )}
                >
                  {d}
                  {destinoComercial === d && <Check className="h-4 w-4 text-brand-600" />}
                </button>
              ))}
            </div>

            {/* 3. Quitar de existencia Ecommerce (switch independiente) */}
            <div className="mt-5 rounded-xl border border-slate-200 p-3">
              <button onClick={() => setQuitarEcommerce((v) => !v)} className="flex w-full items-center justify-between gap-3 text-left">
                <span className="flex items-center gap-2 text-sm font-medium text-slate-700"><Ban className="h-4 w-4 text-rose-500" /> Quitar de existencia Ecommerce</span>
                <span className={cn('relative inline-flex h-5 w-9 shrink-0 items-center rounded-full transition-colors', quitarEcommerce ? 'bg-brand-600' : 'bg-slate-200')}>
                  <span className={cn('inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform', quitarEcommerce ? 'translate-x-4' : 'translate-x-0.5')} />
                </span>
              </button>
              {quitarEcommerce && (
                <p className="mt-2 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-700">
                  Este producto dejará de estar disponible en Ecommerce, pero permanecerá disponible para venta física en tienda.
                </p>
              )}
            </div>
            ...
            <div className="mt-5 flex justify-end gap-2">
              ...
              <Button variant="danger" icon={<X className="h-4 w-4" />} disabled={!rejectReason || !destinoComercial} onClick={confirmReject}>
                Confirmar rechazo
              </Button>
            </div>
          </div>
      ...
      {/* Confirmación — acción independiente "Quitar de existencia Ecommerce" */}
      {confirmEcommerceOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-6" onClick={() => setConfirmEcommerceOpen(false)}>
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-pop" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-2">
              <Ban className="h-5 w-5 text-rose-600" />
              <h3 className="text-base font-semibold text-slate-900">Quitar de existencia Ecommerce</h3>
            </div>
            <p className="mt-2 text-sm text-slate-600">
              Este producto dejará de estar disponible en Ecommerce, pero permanecerá disponible para venta física en tienda.
            </p>
            <div className="mt-5 flex justify-end gap-2">
              <Button variant="ghost" onClick={() => setConfirmEcommerceOpen(false)}>Cancelar</Button>
              <Button variant="danger" icon={<Ban className="h-4 w-4" />} onClick={quitarDeEcommerce}>Confirmar</Button>
            </div>
          </div>
        </div>
      )}
