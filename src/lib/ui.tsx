...
/** Interruptor on/off reutilizable (color corporativo). */
export function Switch({ on, onToggle, disabled }: { on: boolean; onToggle?: () => void; disabled?: boolean }) {
  return (
    <button
      ...
      onClick={onToggle}
      className={cn(
        'relative inline-flex h-5 w-9 shrink-0 items-center rounded-full transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-300 disabled:cursor-not-allowed disabled:opacity-60',
